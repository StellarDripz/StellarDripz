...
export default function FaucetButton() {
  ...
  useEffect(() => {
    if (prevTxRef.current === "pending" && txInProgress === "success") {
      showToast({ type: "success", title: "Faucet success!", message: "10,000 XLM sent." });
    }
    if (prevTxRef.current === "pending" && txInProgress === "error") {
      showToast({ type: "error", title: "Faucet failed", message: "Check transaction log." });
    }
    ...
    <div className="rounded-2xl border border-white/10 bg-surface-800/60 p-6 backdrop-blur-md animate-slide-up">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-white/50">Get 10,000 testnet XLM with one click</p>
        </div>
      </div>

      <button
        onClick={doFaucetRequest}
        disabled={isPending || isOnMainnet || isOnCooldown}
      ...
      >
        {isPending ? (
          ...
          <span className="flex items-center justify-center gap-2">⏳ Cooldown active</span>
        ) : isOnMainnet ? (
      ...
      <p className="mt-3 text-center text-[11px] text-white/30">Powered by Stellar Friendbot</p>
    </div>
  );
}
