...
/**
 * Standalone Soroban smart contract demo component.
 * Shows counter + greeting with read/write interactions.
 */
export default function SorobanDemo({ contractId }: SorobanDemoProps) {
  ...
  const handleGetCounter = useCallback(async () => {
    ...
    try {
      ...
      showToast({ type: "error", title: "Read counter failed", message: err instanceof Error ? err.message : "Error" });
    } finally {
      setLoading(false);
    }
  ...
  const handleIncrement = useCallback(async () => {
    ...
    try {
      ...
      const { resultValue, events } = await callContractFunction(
        contractId, "increment", [address.toScVal()], state.wallet.publicKey
      );
      setCounter(resultValue ? parseInt(resultValue, 10) : (counter || 0) + 1);
      showToast({ type: "success", title: "Incremented!", message: `New value: ${resultValue}` });

      if (events) {
        for (const evt of events) {
          addContractEvent({
            id: `evt-${Date.now()}-${Math.random().toString(36).slice(2)}`,
            contractId: evt.contractId, topic: evt.topic, value: evt.value,
            ledgerSequence: 0, timestamp: new Date(), txHash: "",
          });
        }
      }
    } catch (err) {
  ...
  const handleSetGreeting = useCallback(async () => {
    ...
    try {
      ...
      const { events } = await callContractFunction(
        contractId, "set_greeting", [address.toScVal(), msgScVal], state.wallet.publicKey
      );
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleIncrement} disabled={loading}
              className="rounded-lg bg-gradient-to-r from-stellar-purple to-stellar-blue px-3 py-1.5 text-xs font-semibold text-white hover:shadow-lg active:scale-95 disabled:opacity-50">+1</button>
          </div>
        </div>
      </div>
