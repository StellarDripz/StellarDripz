...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as {
      contractId?: string; functionName?: string;
      args?: unknown[]; signerAddress?: string;
      signedXdr?: string; simulate?: boolean;
    };

    if (!body.contractId || !body.functionName || !body.signerAddress) {
      return NextResponse.json({ error: "contractId, functionName, signerAddress required" }, { status: 400 });
    }
    ...
    if (body.simulate) {
      const result = await simulateContractCallServer(
        body.contractId, body.functionName, scValArgs, body.signerAddress
      );
    ...
    if (body.signedXdr) {
      ...
      const result = await submitContractInvocation(
        body.signedXdr, body.contractId, body.functionName,
        body.signerAddress, { ip, userAgent: ua }
      );
      return NextResponse.json({ success: true, hash: result.hash, resultValue: result.resultValue });
    }
