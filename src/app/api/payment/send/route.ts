...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as {
      signedXdr?: string; destination?: string; amount?: string;
      assetCode?: string; senderAddress?: string;
    };

    // If no signed XDR, just build the transaction for the frontend
    if (!body.signedXdr) {
      if (!body.senderAddress || !body.destination || !body.amount) {
        return NextResponse.json({ error: "senderAddress, destination, and amount required" }, { status: 400 });
      }
      const { xdr } = await buildPaymentTransaction(
        body.senderAddress, body.destination, body.amount, body.assetCode
      );
    ...
    if (!senderAddress || !destination || !amount) {
      return NextResponse.json({ error: "senderAddress, destination, amount required" }, { status: 400 });
    }
    ...
    const result = await sendPaymentServer(
      senderAddress, signedXdr, destination, amount, assetCode || "XLM", { ip, userAgent: ua }
    );
