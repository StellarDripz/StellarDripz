...
impl DripPool {
    ...
    /// Stake tokens. Pulls tokens from user via cross-contract call to DripToken.
    pub fn stake(env: Env, user: Address, amount: i128) {
        ...
        // --- CROSS-CONTRACT CALL: Transfer tokens from user to pool ---
        ...
        // Use DripToken client to call transfer_from (user must have approved pool)
    ...
    /// Unstake tokens. Returns tokens to user via cross-contract call to DripToken.
    pub fn unstake(env: Env, user: Address, amount: i128) {
        ...
        // --- CROSS-CONTRACT CALL: Return tokens from pool to user ---
        let token_id: Address = s::get_persistent(
            &env, &KEY_TOKEN_ID,
            Address::from_string(&String::from_str(&env, "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF")),
        );
...
mod pool_test {
    ...
    fn test_stake_and_unstake() {
        ...
        // Approve pool to spend user's tokens (required for transfer_from)
        token_client.approve(&user, &contract_id, &5000i128, &999999u32);

        // Stake via cross-contract call
        ...
        // Advance past lock period and unstake
